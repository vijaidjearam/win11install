...
{
write-host "System manufacturer has been detected as HP - so proceeding with HP driver update" -ForegroundColor Green
...
Switch ($stage)
{
    ...
    'windows_debloat'
    {
        ...
        powershell.exe -NoProfile -ExecutionPolicy Bypass -File $env:TEMP\win10_debloat.ps1 -include $env:TEMP\win10_debloat.psm1 -include $env:TEMP\windows_settings.psm1 -preset C:\Windows\Setup\Scripts\debloat_preset.txt -log $LogFileName
    ...
    'cleaning'
    {
        ...
        $LogFileName = $env:TEMP+"\"+(Get-Date).tostring("dd-MM-yyyy-hh-mm-ss")+"_"+ $stage+"_log.log"
        ((New-Object System.Net.WebClient).DownloadString($repopath+'cleaning.ps1'))| Out-File $env:TEMP\cleaning.ps1
